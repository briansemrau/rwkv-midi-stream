...
def token_to_midi_message(utils: VocabUtils, token: str, state: DecodeState) -> Tuple[Optional[mido.Message], DecodeState]:
    ...
    if utils.cfg.unrolled_tokens:
        ...
        if token[0] == "t" and token[1].isdigit():  # wait token
            ...
            state.delta_accum = 0.0
            if velocity > 0:
                return mido.Message("note_on", note=note, velocity=velocity, time=ticks, channel=channel), state
            else:
                return mido.Message("note_off", note=note, velocity=0, time=ticks, channel=channel), state
    return None, state


def str_to_midi_messages(cfg: VocabConfig, utils: VocabUtils, data: str) -> Iterator[mido.Message]:
    state = None
    for token in data.split(" "):
        msg, state = token_to_midi_message(utils, token, state)
        if msg is not None:
            yield msg
...
def convert_str_to_midi(cfg: VocabConfig, data: str, meta_text: str = "Generated by MIDI-LLM-tokenizer") -> mido.MidiFile:
    ...
    for msg in str_to_midi_messages(cfg, utils, data):
        track.append(msg)
